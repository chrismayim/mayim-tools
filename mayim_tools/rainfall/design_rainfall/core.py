...
--------------------------------------------------------------------
    ...
    5min-24h:    short-duration L1 from the 24-hour ratio and regression.
                 L1_dur = XCOEF[s_cluster,dur]*L1_24h + CONST[s_cluster,dur]
                 Source: ShortDurationL_1Regressions.dbf, Eq. 16

    2-7 day:     multiday L1 from the regional regression coefficients.
                 Uses the omega regression, Report Eq. 13.
...
class DesignRainfallEngine:
    ...
    def _short_duration_l1(
        ...
        if row.empty:
            raise ValueError(
                f"No short-duration regression for "
                f"S_CLUSTER={s_cluster}, dur={duration_min}"
            )
        return float(row.iloc[0]["XCOEF"]) * l1_24h + float(row.iloc[0]["CONST"])

    def _multiday_l1(self, region: int, duration_days: int, l1_1day: float) -> float:
        r = self.multiday_regr
        row = r[r["REGION"] == region]
        if row.empty:
            raise ValueError(f"No multiday regression for REGION={region}")
        row = row.iloc[0]
        psi = float(row["THETA"]) + float(row["TAU"]) * (
            duration_days ** float(row["SIGMA"])
        )
        omega = float(row["UPSILON"]) + float(row["KAPPA"]) * (
            duration_days ** float(row["RHO"])
        )
