...
class MayimDockWidget:
    ...
    def _populate_tree(self) -> None:
        """
        Populate the tree widget with all registered categories and their tools.
        Each category becomes a top-level tree node.
        Each algorithm becomes a child node under its category.
        """
        ...
        for category in categories:
            ...
            if algorithms:
                ...
                # Placeholder shown when no tools are implemented yet
    ...
    def _on_item_double_clicked(self, item: QTreeWidgetItem, column: int) -> None:
        ...
        If a tool (algorithm) node is double-clicked, open it in the
        Processing Toolbox dialog.

        :param item: The tree item that was double-clicked
        :param column: The column index (always 0 here)
        """
        import processing
        ...
        if isinstance(data, QgsProcessingAlgorithm):
            # Open the tool's Processing dialog directly
            processing.execAlgorithmDialog(data.id())
            MayimLogger.info(f"Opened tool: {data.displayName()}")
        else:
            # Top-level category clicked — toggle expand/collapse
            item.setExpanded(not item.isExpanded())

    def refresh(self) -> None:
        """
        Refresh the tree widget.
        Call this after registering new categories or tools at runtime.
        """
        self._populate_tree()
