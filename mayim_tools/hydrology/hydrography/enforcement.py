...
def enforce_hydrography(
    ...
    authorised_mask = hydrography_mask & eligible_mask & valid_mask

    if conflict_mask is not None:
        authorised_mask &= ~conflict_mask

    eligible_not_modified = (
        hydrography_mask & valid_mask & eligible_mask & ~authorised_mask
    )

    valid_hydrography = hydrography_mask & valid_mask

    eligible_not_modified = valid_hydrography & eligible_mask & ~authorised_mask

    not_enforced_hydrography = valid_hydrography & ~authorised_mask
    ...
    if conflict_mask is not None:
        excluded_hydrography = valid_hydrography & conflict_mask

    if conflict_mask is not None:
        excluded_hydrography = hydrography_mask & valid_mask & conflict_mask
    ...
    if upstream_area is None:
        ...
        area_factor[authorised_mask] = np.clip(
            upstream_area[authorised_mask] / float(reference_upstream_area),
            0.0,
            1.0,
        )
    ...
    result[authorised_mask] = original_authorised - authorised_depths

    difference = np.zeros_like(
        dem,
        dtype=np.float64,
    )
    difference[valid_mask] = result[valid_mask] - dem[valid_mask]
    difference[~valid_mask] = 0.0

    modified_mask = authorised_mask & (np.abs(difference) > 0.0)
    ...
    enforcement_mask[not_enforced_hydrography] = MASK_ELIGIBLE_NOT_MODIFIED

    # Explicit conflicts override the general non-enforced code.
    enforcement_mask[excluded_hydrography] = MASK_CONFLICT_EXCLUDED
    ...
    modified_coordinates = [
        {
            "row": int(row),
            "column": int(column),
            "original_elevation": float(dem[row, column]),
            "new_elevation": float(result[row, column]),
            "elevation_change": float(difference[row, column]),
            "burn_depth": float(-difference[row, column]),
        }
        for row, column in zip(*np.where(modified_mask))
    ]
    ...
    audit: dict[str, Any] = {
        ...
        "maximum_configured_burn_depth": float(maximum_burn_depth),
        ...
        "input_hydrography_cells": int(np.sum(hydrography_mask & valid_mask)),
        "eligible_hydrography_cells": int(
            np.sum(hydrography_mask & valid_mask & eligible_mask)
        ),
        "authorised_hydrography_cells": int(np.sum(authorised_mask)),
        "eligible_not_modified_cells": int(np.sum(eligible_not_modified)),
        "excluded_hydrography_cells": int(np.sum(excluded_hydrography)),
        "modified_cells": int(np.sum(modified_mask)),
        "conflict_excluded_cells": int(
            np.sum(enforcement_mask == MASK_CONFLICT_EXCLUDED)
        ),
        ...
        "total_signed_change": float(np.sum(difference[valid_mask])),
    ...
    if area_factor is not None:
        ...
        audit["area_factor_minimum"] = (
            float(np.min(valid_area_factors)) if valid_area_factors.size else 0.0
        )
        audit["area_factor_maximum"] = (
            float(np.max(valid_area_factors)) if valid_area_factors.size else 0.0
        )
...
def _validate_inputs(
    ...
    if upstream_area is not None:
        if not isinstance(upstream_area, np.ndarray):
            raise ValueError("upstream_area must be a NumPy array or None.")

        if upstream_area.ndim != 2:
            raise ValueError("upstream_area must be two-dimensional.")

        if upstream_area.shape != dem.shape:
            raise ValueError("upstream_area must have the same shape as dem.")

        if not np.all(np.isfinite(upstream_area) | np.isnan(upstream_area)):
            raise ValueError("upstream_area contains invalid values.")

        if reference_upstream_area is None:
            raise ValueError(
                "reference_upstream_area is required when " "upstream_area is supplied."
            )
        ...
        if np.any(np.isfinite(upstream_area) & (upstream_area < 0.0)):
            raise ValueError("upstream_area cannot contain negative values.")
